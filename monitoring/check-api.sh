...
response_code=$(curl -s -o $host -w "%{response_code}" "$url" )
...
if [ $response_code != '200' ] && [ $current_status = "up" ]; then
	echo "API has gone down"
	sendemail -f $from -t $to -u "API down on $host" -m "$url returned $response_code" 
